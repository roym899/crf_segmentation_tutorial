...
def main() -> None:
    ...
    rows = image.shape[0] # pylint: disable=E1136  # pylint/issues/3139
    cols = image.shape[1] # pylint: disable=E1136  # pylint/issues/3139

    logic_indices = np.zeros_like(image, dtype=np.bool)

    if rows > cols:
        row_indices = np.around(np.linspace(0, rows-1, num=args.out_pixels)).astype(np.int)
        col_indices = row_indices[row_indices < cols]
    else:
        col_indices = np.around(np.linspace(0, cols-1, num=args.out_pixels)).astype(np.int)
    ...
    downsampled_image = image[logic_indices].reshape(len(row_indices), len(col_indices), 3)
