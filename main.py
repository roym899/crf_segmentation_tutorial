...
def segmentation_to_probabilities(segmentation: np.ndarray,
                                  prior_probability: float = 0.7) -> np.ndarray:
    """Convert a segmentation into unary probabilities.

    Converts a pixelwise segmentation into probabilities for each class. The prior_probability
    defines the probabiltiy of the specified class. The remaining probability mass is uniformly
    distributed across the remaining classes.

    Args:
        segmentation: 2D integer array, representing the class per pixel. 0 indicates unknown.
    ...
    for class_id in np.arange(start=0, stop=classes+1):
        ...
        if class_id == 0:
            ...
            probabilities[segm == class_id, 0:class_id-1] = (1 - prior_probability) / (classes - 1)
            probabilities[segm == class_id, class_id:] = (1 - prior_probability) / (classes - 1)
    return probabilities.reshape((rows, cols, classes))

# def interactive_mode(image_path: str,
#                      segmentation_path: typing.Optional[str] = None) -> None:
#     """Run the interactive mode.

#     Show two image views, one in which the unary potentials can be modified with a drawing like
#     interface and one with the resulting CRF-based segmentation updated in a separate thread.
...
def refine_mode(image_path: str,
    ...
    initial_segmentation = crf.load_segmentation(segmentation_path, unknown=np.array([0, 0, 0]))
    ...
    params.kernel_weights = [10,1] # smoothness kernel not helpful for such a coarse image
    ...
    final_probabilities = crf.inference(image, 
...
def main() -> None:
    ...
    parser_refine.add_argument("--output",
                               "-o",
