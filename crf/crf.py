...
@dataclass
class CrfParameters:
    '''Parameter class for fully connected CRF with Gaussian appearance and smoothness kernel.'''
...
def appearance_kernel(x_1: int, y_1: int, p_1: np.ndarray,
                      x_2: int, y_2: int, p_2: np.ndarray,
                      theta_alpha: float, theta_beta: float) -> float:
    ...
    """
    return np.exp(-((x_1-x_2)**2.0+(y_1-y_2)**2.0)/(2*theta_alpha**2.0)
                  - np.sum((p_1-p_2)**2.0)/(2.0*theta_beta**2.0))


def smoothness_kernel(x_1: int, y_1: int, p_1: np.ndarray,
                      x_2: int, y_2: int, p_2: np.ndarray,
                      theta_gamma: float) -> float:
    ...
    """
    return np.exp(- ((x_1-x_2)**2.0+(y_1-y_2)**2.0)/(2.0*theta_gamma**2.0))
...
def normalize(potentials: np.ndarray) -> np.ndarray:
    ...
    Args:
        potentials: Array of size MxNxC, with M rows, N columns and C classes.

    Returns:
        Probability array of size MxNxC (same as potentials), such that the probabilities sum up
        to 1 at each position (m,n).
    """

    # TODO: implement normalization here


def message_passing(image: np.ndarray,
                    current_probabilities: np.ndarray,
                    kernel_functions: typing.List[typing.Callable[[int, int, np.ndarray,
                                                                   int, int, np.ndarray], float]]
                    ) -> np.ndarray:
    """Perform "message passing" as the first step of the inference loop.
    
    Args:
        image: Array of size ROWS x COLUMNS x CHANNELS, representing the image used to compute the
               kernel.
        current_probabilities: Array of size ROWS x COLUMNS x CLASSES, representing the current
                               probabilities.
        kernel_functions: The kernel functions defining the edge potential.
        
    Returns:
        Array of size ROWS x COLUMNS x CLASSES x KERNELS, representing the intermediate result of message
        passing for each kernel.
    """
    ...
    result = np.zeros((current_probabilities.shape[0],
...
def efficient_message_passing(image: np.ndarray,
                              current_probabilities: np.ndarray,
                              spatial_downsampling: float,
                              range_downsampling: float,
                              theta_alpha: float,
                              theta_beta: float,
                              theta_gamma: float) -> np.ndarray:
    """Perform efficient "message passing" by downsampling and convolution in 5D.

    This assumes two kernels: an appearance kernel based on theta_alpha and theta_beta, and 
    a smoothness kernel based on theta_gamma.
    ...
    Args:
        ...
        current_probabilities: Array of size ROWS x COLUMNS x CLASSES, representing the current
                               probabilities.
        spatial_downsampling: Factor to downsample the spatial dimensions for the 5D representation.
        range_downsampling: Factor to downsample the range dimensions for the 5D representation.
    ...
    # compute size of downsampled sampled
    ...
    # precompute indices (understand what these are for)
    indices_list = []
...
def compatibility_transform(q_tilde: np.ndarray, weights: typing.List[float]) -> np.ndarray:
    """Perform compatability transform as part of the inference loop.

    Args:
        q_tilde: Array of size ROWS x COLUMNS x CLASSES x KERNELS, representing the intermediate
                 result of message passing for each kernel.
        weights: Weights of each kernel.

    Returns:
        Array of size ROWS x COLUMNS x CLASSES, representing the result after combining the kernels
        and applying the label compatability function (here: Potts model).
    """
...
def local_update(q_hat: np.ndarray, unary_potential: np.ndarray):
    ...
    Args:
        q_hat: Array of size ROWS x COLUMNS x CLASSES, representing the intermediate result after
               combining the kernels and applying the label compatability function.
        unary_potential: Array of size ROWS x COLUMNS x CLASSES, representing the prior energy for
                         each pixel and class from a different source.
    Returns:
        Array of size ROWS x COLUMNS x CLASSES, representing the probabilities for each pixel and
        class.
    """
...
def inference(image: np.ndarray,
              ...
              params: CrfParameters) -> np.ndarray:
    """Perform inference in fully connected CRF with Gaussian edge potentials.

    Args:
        image: Array of size ROWS x COLUMNS x CHANNELS, representing the image used the features.
        initial_probabilities: Initial pixelwise probabilities for each class. Used to initialize
                               unary potential.
        params: Parameter class for fully connected CRFs (see CrfParameters documentation).
    Return:
        Array of size ROWS x COLS x CLASSES
    """

    # define kernels
    kernels = [lambda x1, y1, p1, x2, y2, p2: appearance_kernel(x1, y1, p1,
                                                                x2, y2, p2,
                                                                params.theta_alpha,
                                                                params.theta_beta),
               lambda x1, y1, p1, x2, y2, p2: smoothness_kernel(x1, y1, p1, x2, y2, p2,
                                                                params.theta_gamma)]
    ...
    for _ in np.arange(params.iterations):
        if params.efficient_message_passing:
            q_tilde = efficient_message_passing(image,
