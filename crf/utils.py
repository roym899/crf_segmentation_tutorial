...
def load_image(image_path: str) -> np.ndarray:
    """Loads an image at a given path as a numpy array.

    Args:
        image_path: Path of the image.

    Returns:
        Array of size MxNxC, where M corresponds to the rows of the image, N to the columns
        and C to the channels.
    """
    image = Image.open(image_path)
    return np.array(image)

def load_segmentation(segmentation_path: str,
                      unknown: typing.Optional[np.array] = None) -> np.ndarray:
    """Loads a segmentation at a given path as a numpy array.

    Args:
        segmentation_path: Path of the segmentation.
        unknown: Color of unlabeled, will be assigned index 0, if none, index 0 still correponds to
                 unknown.

    Returns:
        Array of size MxN, where M corresponds to the rows of the image and N to the columns.
    """
    ...
    seg_colors = seg_rgb.reshape(-1, seg_rgb.shape[2]) # pylint: disable=E1136  # pylint/issues/3139

    colors = np.unique(seg_colors, axis=0)
    seg = np.empty(seg_colors.shape[0], dtype=np.uint8) # pylint: disable=E1136  # pylint/issues/3139
...
def plot_segmentation(segmentation: np.ndarray,
                      ...
                      saveas: typing.Optional[str] = None) -> None:
    """Plots a segmentation.

    Args:
        segmentation: Either 2D array of integers representing the class or 3D array, with last
    ...
    if saveas is not None:
        if image is not None:
            ...
            if segmentation.ndim == 2:
                ...
                plt.imsave(saveas, np.argmax(segmentation, axis=2)+1, vmin=0, vmax=classes)
